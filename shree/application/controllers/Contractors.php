...
class Contractors extends CI_Controller {
	...
	function __construct()
	{
		...
		$this->load->model(array('public_model','security_model','contractors_model'));	
	...
	public function form($id='') // edit the item details based on item id
	{
			if(!empty($id))
			{
				 ...
				 $brands= $this->contractors_model->get_details(array('id'=>$id));
			...
			if($this->form_validation->run() == TRUE)
			{
					...
					{											
						$this->db->where('id',$id);
						$this->db->update(CONTRACTORS,$item_details); //update
					}
					else
						$this->db->insert(CONTRACTORS,$item_details);
						
				$this->session->set_flashdata('message','Data has been updated successfully.'); 
				redirect('contractors');
		}
			$this->load->view('contractors/form',$this->data);
	}
	...
	public function ajax_change_status()
	{
		...
		$status=($this->input->post('status')) ? $this->input->post('status') : FALSE;
			...
			foreach($ids as $id):
					...
					$this->db->update(CONTRACTORS,$data); // update
