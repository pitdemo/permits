...
$table.='<tr style="border:1px solid #ccc;">
    ...
    <td style="vertical-align: bottom" colspan="2"><b>EIP.No : '.$sl_no.'</b></td>
    <td style="border:0px solid #ccc;"   id="t2"   align="center"><img src="'.base_url().'/assets/img/print_symbol.jpg" ></td>
</tr>';
...
$work_permit_nos = (isset($records['work_permit_nos'])) ? wordwrap($records['work_permit_nos'],50,'<br />',true) : '';
...
$loop_count=$records['is_checklist'];
     	...
     	$desc_checklist=$eq_radios=array();
     	...
     	if($checklist_count>0)
     	{
	 		...
	 		$loop_count=(isset($total_equipment_tags['Yes'])) ? $total_equipment_tags['Yes'] : 0;
     	}
   
   $additional_rows=EIP_CHECKLIST_ADDITIONAL_ROWS_START;

   $eq_desc=json_decode($records['equipment_descriptions'],true);

    $k=1;

   if(count($eq_radios)>0)
   {
   		foreach($eq_radios as $key => $value):
			...
			if($value==YES)
			{
				$desc=(isset($equipment_descriptions->$key)) ? $equipment_descriptions->$key : '';

				$keys = array_search($desc, $desc_checklist);

				if(is_numeric($keys))
			 	{
			 		$desc=$zones_fetch[$keys]['equipment_name'];
			 	}

			 	$tag_nos = (isset($equipment_tag_nos->$key)) ? $equipment_tag_nos->$key : '';

			 	$isolate_type=(isset($isolate_types->$key)) ? $isolate_types->$key : '';
...
$table.='<tr>
    ...
    <th style="border:1px solid #ccc;" >PA Lock</th>
     <th style="border:1px solid #ccc;" >PA TAG</th>
      ...
      $na_active='checked="checked"';
		...
		$table.='<tr><td style="border:1px solid #ccc;" colspan="1">4</td><td style="border:1px solid #ccc;" colspan="6">Have equipment/pipeliness de-pressurized (vent Open) process material(s) emptied out and drained fully?<td style="border:1px solid #ccc;" colspan="3" align="center"><input type="radio" disabled name="is_isoloation_permit" class="radio_button on_off" '.$yes_active.' />YES&nbsp;<input type="radio" disabled name="is_isoloation_permit" class="radio_button on_off" '.$no_active.' />NO&nbsp;<input type="radio" disabled name="is_isoloation_permit" class="radio_button on_off" '.$na_active.' />N/A</td></tr>';
		 ...
		 $first_name = '';
			...
			$self_coordinated=(isset($records['self_coordinated']) && $records['self_coordinated']!='') ? $records['self_coordinated'] : '____';

			$ccr_person=(isset($records['ccr_person']) && $records['ccr_person']!='') ? $records['ccr_person'] : '____';

			$table.='<tr><td style="border:1px solid #ccc;" colspan="7">(E) Performing Authority<br>I understand clause A,B,C&D, I shall maintain conditions as required. <br /><b>Self '.$self_coordinated.' Coordinated with CCR Operator '.$ccr_person.' to give the equipment in local mode and forcibly started the equipment and found that there is no supply in the equipment</b></td>
			...
			$table.='<tr><td colspan="40"><table align="center" width="1024px" style="font-family:Arial, Helvetica, sans-serif;width:100%;font-size:10px !important; 	margin:0 auto;border-collapse:collapse;" >';

			$table.='<tr rowspan="2"><th rowspan="2" style="border:1px solid #ccc;width:5% !important;">SL.No</th><th  style="border:1px solid #ccc;"  rowspan="2" colspan="2">Eq.Tag No.</th><th style="border:1px solid #ccc;" rowspan="2" >Logo lock & Tag No.</th><th style="border:1px solid #ccc;" colspan="3" >Energization</th><th style="border:1px solid #ccc;" colspan="6">Re‐Isolation</th> </tr>';
			$table.='<tr><th style="border:1px solid #ccc;">Sign of PA</th>
			    <th style="border:1px solid #ccc;">Sign of IA with date and time</th>
			    <th style="border:1px solid #ccc;">Sign of Isolator with date and time</th>
			    <th style="border:1px solid #ccc;">Sign of PA</th>
			    <th style="border:1px solid #ccc;">PA Lock No</th>
			    <th style="border:1px solid #ccc;">PA Tag No</th>
			    <th style="border:1px solid #ccc;">ISO Lock</th>
			    <th style="border:1px solid #ccc;">Sign of IA with date and time</th>
			    <th style="border:1px solid #ccc;">Sign of Isolator with date and time</th>
			</tr>';
				...
				foreach($temporary_tag_nos_full as $i => $val)
				{
					...
					if($temporary_tag_no=='')
					{
						...
						$show_tmp_tag_nos=$temporary_tag_no;
		...
		{		 
			...
			$issuing_authority_3_name=(isset($records['issuing_authority_3_name'])) ? strtoupper($records['issuing_authority_3_name']) : '';

			$table.='<td style="border:1px solid #ccc;" colspan="5">
			        <table style="width:400px;border:0px !important; border-collapse:collapse;" border="0" align="center">
			            <tr><td style="border:1px solid #ccc;border-right:0px solid white;" colspan="2">Connected Energy Sources restored</td></tr>
			            <tr><th>Department</th><th>Digitally Signed</th></tr>
			            <tr><td style="border:1px solid #ccc;">1. Electrical</td><td style="border:1px solid #ccc;border-right:0px solid white;">'.$electrical_name.$department_electrical_completion_users_approval_date.' <br /><b>Name of the IA</b><br />'.$issuing_authority_3_name.'</td></tr>
			            <tr><td style="border:1px solid #ccc;">2. Instrumentation</td><td style="border:1px solid #ccc;border-right:0px solid white;">'.$inst_name.$department_inst_completion_users_approval_date.'</td></tr>

			            <tr><td style="border:1px solid #ccc;">3. Mechanical</td><td style="border:1px solid #ccc;border-right:0px solid white;">'.$mech_name.$department_mech_completion_users_approval_date.'</td></tr>
			            <tr><td style="border:1px solid #ccc;">4. Utilities</td><td style="border:1px solid #ccc;border-right:0px solid white;">'.$uti_name.$department_uti_completion_users_approval_date.'</td></tr>
			            <tr><td style="border:1px solid #ccc;">5. Process</td><td style="border:1px solid #ccc;border-right:0px solid white;">'.$process_name.$department_process_completion_users_approval_date.'</td></tr>
			        </table>
			    </td>
			</tr>';
	}
