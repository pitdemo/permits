           ...
           # echo '<br /> S : '.$is_show_button.' = '.$show_flag.' - '.$approval_status;
            ...
            if($approval_status<13)
            {
                ...
                else
                {
                  ...
                  $is_extended_sa_approval = $is_extended_ia_approval = '';
                  ...
                  if($approval_status>=4)
                  {
                    ...
                    if(strtolower($status)=='extended')
                    {
                      ...
                      end($fil); $fil_key=key($fil); #echo 'FF '.$fil_key;
                      ...
                      for($r=0;$r<count($range);$r++)
                      {
                        ...
                        $is_show_extended_button=1; $is_extended_sa_approval=''; $is_extended_ia_approval=0; $is_extended=0;

                        $show_extend_field=($r+1);      

                        #echo 'fF '.$exten_time_period;
                            		...
                            		else 
                            		{
                            			...
                            			if($extended_safety_from_approval_status->$range[$r]==WAITING || $extended_issuing_from_approval_status->$range[$r]==WAITING)
                            			{
                                    		...
                                    		if($extended_issuing_from_authority->$range[$r]==$user_id && $ex=='') 
    	                        			{
    	                        				...
    	                        				/*else if($extended_issuing_from_approval_status->$range[$r]=='')
    	                        				{
    	                        					$is_show_extended_button=1;
                      ...
                      #echo '<br /> Show Button '.$is_show_extended_button.' = '.$is_extended_ia_approval.' = EXT '.$show_extend_field;
                    ...
                    else if(!in_array($approval_status,array(3,5,6,7,8,)) || ($cancellation_performing_id==$user_id && $cancellation_issuing_date==''))  
                    {
                        $show_flag=2;
                    echo '<button class="btn btn-sm btn-primary show_button"  value="show" type="submit"><i class="fa fa-dot-circle-o"></i> Submit</button>'; }
...
<script type="text/javascript">   
  ...
  $(document).ready(function() {   
    ...
    if(!empty($record_id))
    {
      if($show_flag==1 || $approval_status==13)
      {
    ?>
    ...
    ?>
      $('input,select').attr('disabled',true);
    <?php 
      //if( $user_id==$acceptance_performing_id && $readonly===true && !in_array($approval_status,array(8,6)))
      //{
    ?>
        $('.status').removeAttr('disabled');  //changed by today Anand
    <?php
      //}
      ...
      if(strtolower($status)=='extended')
      {
          ...
          if($show_extend_field>=0)
          {
              ...
              if($is_extended_ia_approval>=1)
              {
       ?>       
              console.log('Inside True <?php echo $is_extended_ia_approval; ?><?php echo $range[$show_extend_field-1].$show_extend_field; ?>');

              var time_period = $('.extended_time_period<?php echo $show_extend_field; ?>').val();

              $('.status').removeAttr('disabled').attr('readonly',true);      

              $('.status:checked').trigger('click');  

              var is_time_period_approval = $('.schedule_'+time_period+'<?php echo $show_extend_field; ?>').attr('data-ia-approval');

              console.log('Is time period approval : '+is_time_period_approval+' Time <?php echo $show_extend_field; ?> Period : '+time_period);

              if(is_time_period_approval=='' || is_time_period_approval=='Waiting')
              $('.schedule_'+time_period+'<?php echo $show_extend_field; ?>').trigger('change').removeAttr('disabled');             
              else if(is_time_period_approval=='Approved')
              { 
                  
                      if(time_period=='to')
                      {
                        console.log('input[name="schedule_date[<?php echo $range[$show_extend_field]; ?>]"]');

                        $('input[name="schedule_date[<?php echo $range[$show_extend_field]; ?>]"]').removeAttr('disabled');
                      }  
                      else
                      {  
                          if(time_period=='from')
                          {
                            $('.schedule_to<?php echo $show_extend_field; ?>').removeAttr('disabled');

                            //Activate next date picker
                            if($('input[name="schedule_date[<?php echo $range[$show_extend_field]; ?>]"]').length>0)
                             $('input[name="schedule_date[<?php echo $range[$show_extend_field]; ?>]"]').removeAttr('disabled');
                          } 
                      }
              ...
              $('.schedule_'+time_period+'<?php echo $show_extend_field; ?>').trigger('change');             
       <?php
                if($is_extended_ia_approval==2)  //For IA User only
                {
       ?>
                  console.log('IA '+'.extended_ia_'+time_period+'<?php echo $show_extend_field; ?>');
                   $('.extended_ia_'+time_period+'<?php echo $show_extend_field; ?>').attr('disabled',true);         
       <?php             
                }
              }
              else if($is_extended_sa_approval==1)
              {
       ?>     
              ...
              show_extended_field = '<?php echo $show_extend_field; ?>'; 
              
              $('.extended_oxygen_'+time_period+show_extended_field).removeAttr('disabled'); 

              $('.extended_gases_'+time_period+show_extended_field).removeAttr('disabled');

              $('.extended_carbon_'+time_period+show_extended_field).removeAttr('disabled');
       <?php
              }
              else if($is_show_extended_button==0)
              {
        ?>
                   console.log('No action <?php echo $show_extend_field; ?>');
                   $('input[name="schedule_date[<?php echo $range[$show_extend_field-1]; ?>]"]').removeAttr('disabled');
        <?php        
              }
              else if($is_show_extended_button==2)
              { 
        ?>        
                  console.log('.schedule_to<?php echo $show_extend_field; ?>');
      ...
      else
      {
        ...
        <?php
            ...
            {
        ?>
              $('#cancellation_issuing_id,.status,#self_cancellation_description').removeAttr('disabled');   //,.status Removed on 12-06     
        <?php
            }
          }
      }
        ...
        if($('.status:checked').val()!='Extended')
        {
          ...
          {         
		            ...
		            else
		            {
		            	...
		            	{		        
                      if(extend_time_period=='from' && $('.schedule_to'+data_id).val()=='')   //If TO time is not used 
                      sa_approval_status=' ';  
