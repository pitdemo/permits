...
class Jobs extends CI_Controller {
	...
	function __construct()
	{
		...
		/*$req=array(
					'sender'=>'anantha.kumar@galaxyweblinks.in',
                    'to'=>'ananthakumar7@gmail.com',
                    //'to'=>'swathilakshmi.s@galaxyweblinks.in',
                    'subject'=>'MCT - Passssssssword Reset',
				...
				$req['mail_content']='Mail Sending From Local Host';
		...
		#${"a"}='Tester';echo '<br /> AA '.$a; exit;

		
		/*$resp=$this->public_model->send_sms(array('sender'=>158,'receiver'=>157,'msg_type'=>IATOPA_ACCEPT_EXTEND_APPROVAL,'permit_type'=>'General Work Permit','permit_no'=>'001'));
	...
	{ 
        ...
        if($update !==FALSE && $this->uri->segment($update+1))
        {
            ...
            $this->data['eips'] = $this->public_model->get_data(array('select'=>'id,section,status','where_condition'=>' (status = "'.STATUS_OPENED.'"  AND zone_id = "'.$records['zone_id'].'" AND DATE(date_end)>=DATE(NOW())) '.$job_isolations_where,'table'=>JOBSISOLATION));
	...
	$this->data['zones'] = $this->public_model->get_data(array('table'=>ZONES,'select'=>'name,id','where_condition'=>'status = "'.STATUS_ACTIVE.'"'));	
		...
		{
			$qry=$this->db->query("SELECT max(id)+1 as sl_no FROM ".$this->db->dbprefix.JOBSISOLATION);
	   ...
	   $qry=$this->public_model->get_data(array('select'=>'id,first_name,user_role','where_condition'=>$where,'table'=>USERS));
		...
		$department_id="'".EIP_PRODUCTION."','".EIP_PACKING_OPERATION."'";
	...
	public function form_action()
	{
		...
		$array_fields=array('precautions_text','issuing_authority_approval_status','precautions_options','hazards_options','precautions','hazards','issuing_authority','no_of_persons','performing_authority','schedule_from_time','schedule_to_time','schedule_date','reference_code','extended_contractors_id','extended_others_contractors_id','hot_work_checklists','height_work_checklists','completion_checklist');
		...
		else
		{
			...
			if($show_button!='reject')
			{
				...
				else if($show_button=='approveIA') //IA approve his job
				{
					...
					$job_qry=$this->public_model->get_data(array('select'=>'id,acceptance_issuing_id,cancellation_issuing_id,approval_status,status','where_condition'=>'id ="'.$job_id.'"','table'=>JOBS));
		...
		if(!empty($self_cancellation_description) && strtolower($status)!='cancellation' && $show_button!='reject')
		{
		...
		#echo '<br /> MSg : '.$msg.' - '.$show_button.' - '.$status.' - '.$user_id.' = '.$msg_type; exit;
