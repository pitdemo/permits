...
<body>';
   ...
   $subcontractor = (isset($records['contractor_id'])) ? strtoupper($records["sub_contractor"]) : '';
	...
	if(in_array(8,$permit_types))
	{
		...
		<td align="left"  style="'.$td_border.'"><b>Isolator Name</b></td>
		<td align="left"  style="'.$td_border.'"><b>Name of the IA & Sign Date</b></td>
    	</tr>';
		...
		foreach($equipment_tag_nos as $i => $value)
		{
			if($value!='')
			{
				...
				<td align="left"  style="'.$td_border.'">'.$isolated_ia_name.' '.$isolated_name_approval_datetime.'</td>
				</tr>';
			}
	...
	if(in_array(3,$permit_types))
	{
		...
		$checkbox = (isset($precautions_data[27]) && $precautions_data[27]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='<tr><td align="left" colspan="5" style="'.$valign.$td_border.'">'.$checkbox.'Welding Helmet';
		
		$checkbox = (isset($precautions_data[28]) && $precautions_data[28]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='&nbsp;&nbsp;&nbsp;'.$checkbox.'Leather Hand gloves';
		$checkbox = (isset($precautions_data[29]) && $precautions_data[29]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='&nbsp;&nbsp;&nbsp;'.$checkbox.'Leather Apron';
		$checkbox = (isset($precautions_data[30]) && $precautions_data[30]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='&nbsp;&nbsp;&nbsp;'.$checkbox.'Hand Sleeves';
		$checkbox = (isset($precautions_data[31]) && $precautions_data[31]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='&nbsp;&nbsp;&nbsp;'.$checkbox.'Leg Guard';
		$checkbox = (isset($precautions_data[32]) && $precautions_data[32]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
		$table.='&nbsp;&nbsp;&nbsp;'.$checkbox.'Welding Goggles for Helper';
		$checkbox =(isset($precautions_data[33]) && $precautions_data[33]=='y') ? checkbox(array('status'=>'yes','style'=>'float: right;vertical-align: top;')) : checkbox(array('status'=>'no','style'=>'float: right;vertical-align: top;'));
	...
	if(in_array(7,$permit_types))
	{
		...
		$labels=array(1=>'Equipment properly drained / Depressurized',2=>'Disconnected Inlet, Outlet lines and isolate equipment',3=>'Vent / Manholes are kept open to maintain proper ventilation',4=>'Only trained personnel were engaged & register was maintained for entry & exit of person with time.',5=>'Standby Personnel/Entry supervisor,. is available',6=>'Oxygen level is measured and find………………( 19.5% - 23.5% is available)',7=>'Flammable gases and vapours reading: ≤ 5% LEL',8=>'Toxic gases and vapours reading: ≤ PEL values',9=>'Inside temperature closed to room temperature and ambient air comfortable for working.',10=>'Recommended communication system',11=>'Safe Illumination provided (24-volt hand lamps)',12=>'Additional Measures.');
	...
	$table.='<tr><td colspan="3" style="'.$td_border.$valign.'"><b>PA Work Completion/Cancellation</b><br />Work completed, all persons are withdrawn and material removed from the area <br /><br /><b>Performing Authority: </b>'.$cancellation_performing_name.' '.$cancellation_performing_date.'</td>';
	
	$table.='<td colspan="2" style="'.$td_border.$valign.'"><b>IA Work Completion/Cancellation</b><br />I have inspected the work area and declare the work for which the permit was issued has been properly. <br /><br /><b>Issuing Authority: </b>'.$cancellation_issuing_name.' '.$cancellation_issuing_date.'</td></tr>';
