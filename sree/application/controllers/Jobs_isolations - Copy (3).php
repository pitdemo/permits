...
class Jobs_isolations extends CI_Controller
 {
	...
	public function ajax_fetch_data()
	{
		...
		if($approval_status !==FALSE && $this->uri->segment($approval_status+1))
		{
		 ...
		 # echo $where_condition; exit;
		  ...
		  if($search_value!='')
		  {
			  $where_condition.=" (j.section like '%".$search_value."%' OR j.area like '%".$search_value."%' OR j.equipment_descriptions like '%".$search_value."%' ) AND ";
		  }
		...
		if($totalFiltered>0)
		{
			...
			foreach($records as $record)
			{
				...
				$json[$j]['id']='<a href="'.$redirect.'" data-isoloation_permit_no="'.$id.'" title="Energy Isolation Permit Form" >'.'#'.$id.'</a>';
			...
			else if($approval_status==2 || $approval_status==3) 	//Stage C
			{
				...
				#,'isolated_tagno3','isolated_name_approval','isolated_name','isolated_name_approval_datetime','isolated_ia_name'
					...
					else if($c==$i)
					{
						 ...
						 $this->public_model->send_sms(array('sender'=>$sender,'receiver'=>$receiver,'msg_type'=>$msg_type,'permit_type'=>'EIP','permit_no'=>$id));
	...
	//'is_isoloation_drained_fully_options',
		...
		'temporary_iso_signdates','temporary_re_pa','temporary_re_ia','temporary_re_iso','temporary_re_pa_signdates','temporary_re_ia_signdates','temporary_re_iso_signdates','department_completion_users','department_completion_users_approval_dates','equipment_radio','equipment_remarks','isolated_ia_name');
		
		$skip_fields=array('id','submit','is_popup_submit','show_button');
		...
		$ret=array('id'=>$id,'print_out'=>$print_out);
	...
	public function check_assigned_permits_status($id)
	{
		...
		{	
			...
			$opened_jobs = $this->public_model->get_data(array('table'=>JOBSISOLATIONRELATIONS,'select'=>'job_id','where_condition'=>$where));
	...
	**********************************************************************************************/	
	...
	public function ajax_get_isolation_users()
	{
		...
		$this->db->select('u.first_name,u.id');
		...
		#$this->db->where('ui.isolation_id',$isolation_type_id);
	...
	public function get_assigned_permits($id)
	{
		...
		return array('job_descriptions'=>$job_descriptions,'work_permit_nos'=>$work_permit_nos,'job_names'=>$job_names);
	...
	public function ajax_get_isolations()
	{
		...
		$qry=$this->public_model->get_data(array('select'=>'id,section,status','where_condition'=>$where,'table'=>JOBSISOLATION));
		...
		if($qry->num_rows()>0)
		{
			...
			{
				$options.='<option value="'.$fet['id'].'">'.$fet['section'].'('.$fet['id'].')</option>';
			}
