...
class Checklists extends CI_Controller {
	...
	{        
        ...
        $c_id = array_search('permit_type_id',$this->uri->segment_array());
        $id='';
        ...
        if($c_id !==FALSE && $this->uri->segment($c_id+1))
        {
			...
			$where.=' AND pc.permit_type_id = "'.$id.'"';
        }  
		
		#$this->data['users'] = $this->public_model->get_data(array('select'=>'*','where_condition'=>$where,'table'=>PERMITS_CHECKLISTS));    

		$this->data['users']=$this->public_model->join_fetch_data(array('select'=>'pc.*,pt.name as permit_name','table1'=>PERMITS_CHECKLISTS.' pc','table2'=>PERMITSTYPES.' pt','join_type'=>'inner','join_on'=>'pt.id=pc.permit_type_id','where'=>$where,'num_rows'=>false));
	...
	public function copermittees_form($id='') // edit the item details based on item id
	{
			if(!empty($id))
			{
				 $id = base64_decode($id);
				
				 $brands= $this->public_model->get_data(array('select'=>'*','where_condition'=>'id="'.$id.'"','table'=>COPERMITTEES)); 
			...
			if($this->form_validation->run() == TRUE)
			{
					...
					{											
						$this->db->where('id',$id);
						$this->db->update(COPERMITTEES,$item_details); //update
					}
					else
						$this->db->insert(COPERMITTEES,$item_details);
						


				$this->session->set_flashdata('message','Data has been updated successfully.'); 
				redirect('checklists/copermittees');
			}
			$this->data['menu']='checklists';

			$this->load->view('checklists/copermittees_form',$this->data);
	}

	public function ajax_update_copermittees_status()
	{
		...
		$status=($this->input->post('status')) ? $this->input->post('status') : FALSE;
			...
			foreach($ids as $id):
					...
					$this->db->update(COPERMITTEES,$data); // update
	...
	public function permit_checklists_form($id='')
	{
		...
		$req=array(
		  ...
		  if($update !==FALSE && $this->uri->segment($update+1))
		  {
		    ...
		    $this->form_validation->set_rules('name', 'Name', 'trim|required');
			$this->form_validation->set_rules('permit_type_id', 'Permit type', 'trim|required');
			$this->form_validation->set_error_delimiters('<div class="error-val">', '</div>');	
			if($this->form_validation->run() == TRUE)
			{
				...
				$this->session->set_flashdata('message','Data has been updated successfully.'); 
				redirect('checklists/permit_checklists/permit_type_id/'.$this->input->post('permit_type_id'));
			}
