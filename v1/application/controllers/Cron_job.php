...
class Cron_job extends CI_Controller {
	...
	public function backup()
	{
		...
		$where="WHERE DATEDIFF(DATE(NOW()),DATE(created))>60";
		...
		//Deleting PDF files
		delete_files('./uploads/permits/', TRUE);
		delete_files('./uploads/eip/', TRUE);
		...
		$where="WHERE DATEDIFF(DATE(NOW()),DATE(created))>60 AND status='closed'";

			$qry=$this->db->query("SELECT id FROM dml_".JOBSISOLATION." ".$where." ORDER BY ID ASC");
			...
			if($nums>0)
			{
				...
				$ids = implode(',',array_column($fetch,'id'));				

				$where="WHERE jobs_isolations_id IN(".$ids.")";

				//$qry=$this->db->query("INSERT INTO dml_".JOBSISOLATION_REISOLATIONS."_bk2 SELECT * FROM dml_".JOBSISOLATION_REISOLATIONS." ".$where." ORDER BY jobs_isolations_id ASC");

				//$this->db->query("DELETE FROM dml_".JOBSISOLATION_REISOLATIONS." ".$where);

				$where="WHERE id IN(".$ids.")";

				//$this->db->query("INSERT INTO dml_".JOBSISOLATION."_bk2 SELECT * FROM dml_".JOBSISOLATION." ".$where." ORDER BY ID ASC");

				//$this->db->query("DELETE FROM dml_".JOBSISOLATION." ".$where);

				$where="WHERE jobs_isoloations_id IN(".$ids.")";	
				
				//$this->db->query("INSERT INTO dml_".JOBSISOLATIONRELATIONS."_bk2 SELECT * FROM dml_".JOBSISOLATIONRELATIONS." ".$where." ORDER BY jobs_isoloations_id ASC");

				//$this->db->query("DELETE FROM dml_".JOBSISOLATIONRELATIONS." ".$where);			
			}
