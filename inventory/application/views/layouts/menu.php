...
<div id="sidebar"> 
    ...
    if($this->session->userdata('session_user_type')== $user_roles[0]  || array_intersect($array_order,$allowed_tab_permissions))
      {
      ?>
      <li <?php echo (in_array($menu,array('items','customers','suppliers','ledger','items_group'))?'class="active"':"");?>> <a href="#" class="dropdown-toggle"> <i class="icon-double-angle-right"></i>
     
         <span>Master</span> <b class="arrow icon-angle-down"></b> </a>    <?php } ?>
         <ul class="submenu" <?php echo (in_array($menu,array('items','customers','suppliers','ledger','items_group'))?'style="display: block;"':"");?>>
  ...
  //  exit;
    ...
    if($this->session->userdata('session_user_type')== $user_roles[0]  ||  in_array($user_permissions[14],$allowed_tab_permissions))
     ...
     <!--Code for Add itemgroups submenu - soundarya -->
      <?php
  //print_r($user_permissions);
  //  exit;
            //$user_permissions[1] is items(items tab is only visible to authorized users)
    if($this->session->userdata('session_user_type')== $user_roles[0] || $this->session->userdata('session_user_type')== $user_roles[1] ||  in_array($user_permissions[16],$allowed_tab_permissions))
       {
    
       ?>
         <li <?php echo ($menu == "items_group")?'class="active"':"";?>> <a href="<?php echo base_url('items_group');?>"> <i class="icon-double-angle-right"></i> <span>Items Group</span> </a> </li>
          <?php 
