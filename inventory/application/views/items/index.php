...
<div class="container-fluid" id="main-container"> <a id="menu-toggler" href="#"> <span></span> </a>
  ...
  <div id="main-content" class="clearfix">
    ...
    <div class="alert alert-warning" style="width:500px;margin:0 auto">
            ...
            <ul>
            <li>The item-code is unique and auto generated by system based on the items. </li>
            <li>You can upload bulk item details through csv file.</li>
            <li>When you click on the stock values in the column "Stock" you will be able to view the item "Purchase history" and "Sales history."</li>
            </ul>
            </span></div>
      ...
      <div class="row-fluid"> 
        ...
        <button class="btn btn-info" style="float:right; margin-left:10px; margin-bottom:10px;" onClick="window.location.href='<?php echo base_url();?>items/create_item'" type="button">+ Create Item</button>
          <button class="btn btn-info" style="float:right; margin-left:10px; margin-bottom:10px;" onClick="window.location.href='<?php echo base_url();?>items/bulk_insert'" type="button">Import data from CSV</button>
            	 ...
            	 <table id="table_report" class="table table-striped table-bordered table-hover">
                  <thead>
                    <tr>
                      ...
                      <th class="center" width="10%">Item Code</th>
                      <th class="center" width="20%">Name</th>
                      <th class="center" width="10%">Date</th>
                       <th class="center" width="10%">Item Type</th>
...
$(function() 
{
	...
	$('#table_report').dataTable(
	{
		...
		<?php if($this->session->userdata('session_user_type')=='super_admin'){?>
		 ...
		 <?php } ?>
		"ajax": { url :"<?php echo base_url()?>items/datafetch/", type: "post"},
		"columns": 
		[
			{ "data": "item_code" },
			{ "data": "item_name" },
			...
			{		
				"mRender": 	function (data,type,full)                              
				{
					...
					//return '<a href="javascript:void(0)" onclick="change_status('+id+',this)"><img id="status_img" src="<?php echo base_url()?>assets/images/active.png" title="Change Inactive" alt="no image"></a>';
					return '<a href=<?php echo base_url()?>items/edit_item/'+encodedString+' class="btn btn-mini btn-info" type="button" title="Edit"><i class="icon-edit bigger-120"></i></a><button type="button" style="cursor:pointer;" onClick="delete_item('+id+');" class="btn btn-mini btn-danger" title="Delete"><i class="icon-trash bigger-120"></i></button>';
				}
			},
			<?php } ?>
...
function formatDate(date) {
    ...
    return [year, month, day].join('-');
			...
			function delete_item(item_id,element)
			{
				...
				if (choice==true)
				{
					...
					success: function(){
						...
						{
							$("#succ_msg").show();
							$("#succ_msg").html('<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>Item has been deleted successfully.<br>');
						}
						else {
							$("#page-content").prepend('<div id="succ_msg" class="alert alert-info"><button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>Item has been deleted successfully.<br></div>');
						}
					}
					});										
				}
			...
			function change_status(item_id,element)
			{
                ...
                {
				$.ajax({
					url: '<?php echo base_url();?>items/change_status',
					type: 'POST',
					data: { item_id:item_id },
					...
					success: function(msg){
						...
						{
							$("#succ_msg").show();
							$("#succ_msg").html('<button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>Item status has been Changed successfully.<br>');
						}
						else {
							$("#page-content").prepend('<div id="succ_msg" class="alert alert-info"><button type="button" class="close" data-dismiss="alert"><i class="icon-remove"></i></button>Item status has been Changed successfully.<br></div>');
						}
