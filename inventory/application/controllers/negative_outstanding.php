...
/**********************************************************************************************
 * Filename       : negative_outstanding.php
 * Project        : Accounting Software
 * Creation Date  : 16-07-2015
 * Author         : G.Uma Maheswari
 * Description    : Manage the Negative_outstanding report
*********************************************************************************************/	
class Negative_outstanding extends CI_Controller {

	public function __construct() 
	{
		parent::__construct();
		$this->load->model(array('security_model','sales_model','supplier_model','items_model','customer_model','notes_model','outstanding_model','negative_outstanding_model')); //load the model files here
		$this->security_model->chk_admin_login(); // check admin logged in or not
	}
   public function index()
    {
        redirect('negative_outstanding/lists');
    }
	public function lists() // lists
    {
		...
		$this->data['menu'] = 'negative outstanding';
		$this->data['persons'] = $this->negative_outstanding_model->get_active_customer_lists_all($segment_array);
		$this->data['person_suppliers'] = $this->negative_outstanding_model->get_active_supplier_lists_all($segment_array);
		$this->data['salesman_list_customer'] = $this->negative_outstanding_model->get_active_sales_person_customer_lists();
		$this->data['salesman_list_supplier'] = $this->negative_outstanding_model->get_active_sales_person_supplier_lists();
		
		$this->load->view('negative_outstanding/lists',$this->data);
    }
	...
	public function salesman_list_customer($salesman_list_customer='',$persons_arr='')
	{
		if($persons_arr!='')
		{
		 ...
		 $persons = $this->negative_outstanding_model->get_active_customer_lists($salesman_list_customer);
	...
	{
		 $person_supplier = $this->negative_outstanding_model->get_active_supplier_lists($salesman_list_supplier);
 ...
 public function ajax_get_customers_list() // load bulk records to jquery data table using ajax
		 {
            $segment_array = $this->uri->segment_array();
            $lists = $this->negative_outstanding_model->get_customers_lists($segment_array);
			//print_r( $lists);exit;
			$this->data['menu'] = 'negative outstanding';
			$this->data['active_tab'] = 'negative outstanding';
			$seg_url = array_search('user_type',$segment_array);
             $sales_list = array();
                foreach($lists as $list)
                {
                    ...
                    {
                      $supplier_details   = $this->negative_outstanding_model->get_supplier_details($list['user_id']);
                      $sales_list[$list['id']]['user_name'] = $supplier_details['supplier_name'];
                    }
                    else
                    {
                     $customer_details   = $this->negative_outstanding_model->get_customer_details($list['user_id']);
				...
				#print_r($sales_list);exit;
                        foreach($sales_list as $user)
                        {
							...
							if($outstanding<0)
							{	
								$lists = $this->negative_outstanding_model->get_customer_history($user['id']);
		...
		#echo number_format($sum_sales_aboveonetwenty,2); exit;		
           ...
           if($option!==FALSE)
		   {
                ...
                else if($option_type=='csv')
                {
                    ...
                    header("Content-Disposition: attachment; filename=negative_outstanding_report.csv");
                    // Disable caching
